...
static int parse_url(char *src_url, int *https, char *host, char *port, char *url)
{
	...
	if(strncmp(src_url, "http://", 7) == 0) {
		p1 = &src_url[7];
		*https = 0;
	} else if(strncmp(src_url, "ws://", 5) == 0) {
		p1 = &src_url[5];
		*https = 0;
	} else if(strncmp(src_url, "https://", 8) == 0) {
		p1 = &src_url[8];
		*https = 1;
	} else if(strncmp(src_url, "wss://", 6) == 0) {
		p1 = &src_url[6];
		*https = 1;
	} else {
		p1 = &src_url[0];
...
//  debug("request: %s", request);
	...
	while(1) {
		...
		// debug("read(%ld): |%s| ", hi->r_len, hi->r_buf);
		// debug("read(%ld) ... ", hi->r_len);
	...
	/*
	debug("status: %d ", hi->response.status);
	debug("cookie: %s ", hi->response.cookie);
	debug("location: %s ", hi->response.location);
	debug("referrer: %s ", hi->response.referrer);
	debug("length: %ld ", hi->response.content_length);
	debug("body: %ld ", hi->body_len);
	*/
...
//        debug("read(%ld) ", hi->r_len);
	...
	/*
	    ...
	    debug("location: %s ", hi->response.location);
	    debug("referrer: %s ", hi->response.referrer);
	    debug("length: %d ", hi->response.content_length);
	    debug("body: %d ", hi->body_len);
	*/
...
{
	static char        host[256], port[10], dir[1024];
	int         sock_fd, https, verify;
	int         ret, opt;
	socklen_t   slen;
...
int http_write_ws_header(HTTP_INFO *hi)
{
	...
	unsigned char base64_key[(16 / 3 + 1) * 4 + 1];
	int i;
	...
	for(i = 0; i < 16; i++) {
		key[i] = (rand() % (127 - 32)) + 32;
	}

	ret = mbedtls_base64_encode(base64_key, sizeof(base64_key), (size_t *)&len, key, 16);
...
//        debug("read(%ld) ", hi->r_len);
	...
	/*
	    debug("status: %d ", hi->status);
	    debug("cookie: %s ", hi->cookie);
	    debug("location: %s ", hi->location);
	    debug("referrer: %s ", hi->referrer);
	    debug("length: %d ", hi->content_length);
	    debug("body: %d ", hi->body_len);
	*/
