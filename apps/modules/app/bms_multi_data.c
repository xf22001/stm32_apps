 ...
 *   修改日期：2020年04月09日 星期四 14时15分10秒
 *   描    述：
 *
 *================================================================*/
...
static int send_bms_multi_request_response(can_info_t *can_info, multi_packets_info_t *multi_packets_info, bms_data_settings_t *settings)
{
	int ret = 0;
	can_tx_msg_t tx_msg;
...
static int send_bms_multi_data_response(can_info_t *can_info, multi_packets_info_t *multi_packets_info, bms_data_settings_t *settings)
{
	int ret = 0;
	can_tx_msg_t tx_msg;
			...
			switch(request_head->type) {
				case FN_MULTI_REQUEST_TYPE: {//处理多包请求
					...
					if(multi_packets_des->bms_data) {
						send_bms_multi_request_response(can_info, multi_packets_info, settings);
						ret = 0;
					}
				}
				break;
			...
			if(received < multi_packets_des->bms_data_multi_bytes) {
				...
				if(data->index == multi_packets_des->bms_data_multi_next_index) {
					...
					if(data->index < multi_packets_des->bms_data_multi_packets) {
						...
						send_bms_multi_data_response(can_info, multi_packets_info, settings);
					}
				}
...
static int send_multi_request(can_info_t *can_info, multi_packets_info_t *multi_packets_info, bms_data_settings_t *settings)
{
	int ret = 0;
	can_tx_msg_t tx_msg;
	multi_packets_des_t *multi_packets_des = &multi_packets_info->tx_des;
...
static int send_multi_data(can_info_t *can_info, multi_packets_info_t *multi_packets_info, bms_data_settings_t *settings)
{
	int ret = 0;
	can_tx_msg_t tx_msg;
	multi_packets_des_t *multi_packets_des = &multi_packets_info->tx_des;
...
int send_multi_packets(can_info_t *can_info, multi_packets_info_t *multi_packets_info, bms_data_settings_t *settings, uint8_t *data, uint16_t fn, uint16_t bytes, uint16_t packets, uint32_t period)
{
	...
	if(multi_packets_des->bms_data_multi_fn == FN_INVALID) {
		...
		send_multi_request(can_info, multi_packets_info, settings);
		...
		if((multi_packets_des->bms_data_multi_next_index >= 1) && (multi_packets_des->bms_data_multi_next_index <= multi_packets_des->bms_data_multi_packets)) {
			send_multi_data(can_info, multi_packets_info, settings);
